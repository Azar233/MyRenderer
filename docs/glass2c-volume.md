...
`assets/environments/glass_studio.hdr` is an original 256x128 Radiance RGBE
equirectangular environment generated by `tools/GenerateGlassStudioHdr.ps1`.
At renderer startup it is converted into:
...
The Cook-Torrance ambient term now follows the standard split-sum form. A
high-contrast warm key box, cool rim box, and warm floor strip make curved
glass reflections easy to inspect. If the HDR file is unavailable, the same
